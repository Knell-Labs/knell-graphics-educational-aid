...
const Modal: FC<ModalProps> = ({ title, isOpen, onClose, children }) => {
  ...
  return (
    <div
      className={`mx-4 text-black fixed inset-0 flex items-center justify-center z-50 transition-opacity duration-300 ${visibilityClass}`}
    >
      <div className="fixed inset-0 bg-black opacity-50 transition-opacity duration-500"></div>
      <div
        className={`max-w-md bg-white w-1/2-lg rounded-lg shadow-lg z-10 transition-transform transform duration-300 ${
          isOpen ? "translate-y-0" : "translate-y-4 scale-95"
        }`}
      >

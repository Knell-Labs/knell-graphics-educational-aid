...
export function RayCaster({
  ...
  const [mouseCords, setMouseCords] = useState<THREE.Vector2>(
    new THREE.Vector2(),
  );
  ...
  useEffect(() => {
    const handleClick = (event) => {
      const intersect = raycaster.intersectObject(
        world.scene.getObjectByName("grid-plane-hidden-helper"),
      );
...
function ActiveToolOverLay(
  ...
  switch (currTool) {
    ...
    case "cube": {
      ...
      if (
        currCameraPos === CameraDirection.freeDrive ||
      ...
      plane.position.set(
        objectPlacePosition[0],
        objectPlacePosition[1],
        objectPlacePosition[2],
      );
...
function objectPlacingPosition(
  currCameraPos: CameraDirection,
  pointX: number,
  pointY: number,
  pointZ: number,

...
const DropdownButton: React.FC<DropdownButtonProps> = ({
  ...
  switch (variant) {
    ...
    case "primary-link":
      colorClasses =
        "text-white hover:text-indigo-400 no-underline hover:underline"; // Anchor-like appearance
      break;
    case "secondary":
    ...
    case "link":
      colorClasses =
        "text-indigo-500 hover:text-indigo-600 no-underline hover:underline"; // Anchor-like appearance
      break;
    case "disabled":
              ...
              {items.map((item, index) => {
                ...
                if (item.isText) {
                  return (
                    <div
                      key={index}
                      className="px-4 py-2 text-sm leading-5 text-gray-700"
                    >
                      {item.label}
                    </div>
                  );
                ...
                if (item.isFormAction) {
                  return (
                    <form
                      key={index}
                      action={item.formActionUrl}
                      method={item.formMethod}

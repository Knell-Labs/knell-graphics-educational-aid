...
const reducer = (state: State, action: Action): State => {
    switch (action.type) {
        ...
        case 'SET_CONFIRM_PASSWORD': return { ...state, confirmPass:    action.payload };
        case 'VALIDATE_FIELDS': {
            const emailPattern = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i;
            
            let errorMsg = !state.username ? 'Username is required' :
                           !state.email ? 'Email is required' :
                           !emailPattern.test(state.email) ? 'Invalid email address' :
                           !state.password ? 'Password is required' :
                           state.password.length < 8 ? 'Password must have at least 8 characters' :
                           state.password !== state.confirmPass ? 'Passwords do not match' :
                           '';
...
const CreateUserForm: React.FC = () => {
    ...
    return (
        <div className="p-6 space-y-2">
            ...
            <p className={`text-red-500 text-xs opacity-0 ${state.errorMsg ? "opacity-100" : ''}`}>&#x26A3; {state.errorMsg}</p>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label htmlFor="form-username" className="block text-sm font-medium text-gray-700">
                        {"Username"}
                    </label>
                    <input
                        id="form-username"
                ...
                <div>
                    <label htmlFor="form-confirm-password" className="block text-sm font-medium text-gray-700">
                        {"Confirm Password"}
                    </label>
                    <input
